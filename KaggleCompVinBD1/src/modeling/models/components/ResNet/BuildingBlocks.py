...
class ResnetBuildingBlock(Module):
    def __init__(self, size, kernel_size=(1, 1), stride=(1, 1)):
        super().__init__()

        self.C1 = nn.Conv2d(size, size, kernel_size=kernel_size, stride=stride, bias=False)
        self.C1_BN = nn.BatchNorm2d(size)
        self.C1_ACT = nn.ReLU(inplace=True)

        self.C2 = nn.Conv2d(size, size, kernel_size=kernel_size, stride=stride, bias=False)
        self.C2_BN = nn.BatchNorm2d(size)
